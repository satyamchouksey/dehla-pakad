...
function PlayerSeatInner({
  ...
  return (
    <div className={`flex flex-col items-center gap-2 ${!player.connected ? 'opacity-50' : ''}`}>
      {/* Player info */}
      <motion.div
      ...
      >
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[0.6rem] text-white/50">
            Team {player.team} {isDealer ? '• Dealer' : ''}
          </span>
        </div>
        {!player.connected && (
          <span className="text-[0.6rem] text-red-400">⚡ DC</span>
        )}
      </motion.div>

      {/* Face-down cards */}
      <div
        className={`flex ${isVertical ? 'flex-row' : 'flex-row'} items-center justify-center`}
        style={{ gap: '-4px' }}
      >
        ...
        {cardCount > 5 && (
          <span className="text-xs text-white/50 ml-1">+{cardCount - 5}</span>
        )}
      </div>
    </div>
  );
